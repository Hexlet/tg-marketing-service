...
class ChannelParser(BaseParser):
    ...
    async def parse_channel(self, url: str) -> Dict:
        ...
        data = {
            ...
            "creation_date": channel_entity.date.isoformat() if channel_entity.date else None,
        }

        # Сохранение модели канала
        django_channel = await self.save_channel_model(channel_entity, data)

        # Получение полной информации о канале (участники, описание, закреплённое сообщение)
        try:
            ...
            data.update({
                ...
                "description": full_channel.full_chat.about or "Нет описания",
            })
            django_channel.description = data["description"]
            django_channel.subscribers_count = data["participants_count"]
            django_channel.save(update_fields=["description", "subscribers_count"])

            # Обработка закреплённого сообщения
            pinned_msg_id = full_channel.full_chat.pinned_msg_id
            if pinned_msg_id:
                pinned_message = await self.client.get_messages(channel_entity, ids=pinned_msg_id)
                data["pinned_messages"] = [{
                    "text": pinned_message.text or "Нет текста",
                    "id": pinned_message.id,
                    "date": pinned_message.date.isoformat() if pinned_message.date else None,
                }]
