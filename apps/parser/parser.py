...
async def tg_parser(
    ...
    Returns:
        Dict: Результаты парсинга, включая метаданные канала и статистику постов.
    """
    ...
    try:
        ...
        try:
            ...
            django_channel = await TelegramChannel.objects.aget(telegram_id=channel_data["channel_id"])
            ...
            log.info(f"Successfully parsed channel {channel_data['title']} ({channel_data['username']})")
            return data
