...
После обработки endpoint возвращает Inertia `location()` на
`/admin/moderation/` с HTTP-статусом `409`. Результат операции передаётся
через общий prop `flash`.
...
- `verified` — значение `true` включает проверку канала, остальные значения
  трактуются как `false`.

При успешном approve канал публикуется, получает категорию и значение
`verified`, а заявка получает статус `approved`, модератора и время обработки.
...
`flash.error`; успешные операции — через `flash.success`.
