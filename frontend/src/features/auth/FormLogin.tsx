...
const FormLogin: React.FC = () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        if (error.response.status === 404) {
          setServerError('Пользователь с такими данными не найден');
        } else {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className='max-w-sm flex flex-col justify-center m-auto h-screen gap-4 p-5'>
        <div>
        ...
        // onChange={() => setServerError(null)} // Сбрасываем ошибку при изменении
        />
        <InputFieldForm
          type='password'
          name='password'
